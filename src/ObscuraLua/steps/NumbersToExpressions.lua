...
function NumbersToExpressions:init(settings)
    self.ExpressionGenerators = {
        ...
        function(val, depth)                       -- Logarithmic
            if val <= 0 then return false end   -- Logarithms only make sense for positive values
            for base = 2, 10 do
                local exponent = math.log(val) / math.log(base)
                if exponent == math.floor(exponent) then -- Check for a perfect logarithmic expression
                    return Ast.LogExpression(self:CreateNumberExpression(base, depth),
                        self:CreateNumberExpression(exponent, depth), false)
                end
            end
            return false -- Return false if no perfect logarithmic base is found
        end,
        function(val, depth)                       -- Square Root
            if val < 0 then return false end -- Square roots only make sense for non-negative values
            local sqrtVal = math.sqrt(val)
            return Ast.PowExpression(self:CreateNumberExpression(sqrtVal, depth), 
                self:CreateNumberExpression(2, depth), false)
        end,
        function(val, depth)                       -- Absolute Value
            local absVal = math.abs(val)
            return Ast.SubExpression(Ast.NumberExpression(0), self:CreateNumberExpression(absVal, depth), false)
        end,
        function(val, depth)                       -- Negation
            local negVal = -val
            return Ast.NegExpression(self:CreateNumberExpression(negVal, depth), false)
        end,
