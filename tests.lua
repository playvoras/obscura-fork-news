...
-- Require ObscuraLua
local ObscuraLua = require("src.ObscuraLua");
...
ObscuraLua.colors.enabled = not noColors;

-- Apply Obfuscation Pipeline
local pipeline = ObscuraLua.Pipeline:new({
...
print("Performing ObscuraLua Tests ...")
...
local presets = ObscuraLua.Presets;
local testdir = "./tests/"
local failed = {};
ObscuraLua.Logger.logLevel = ObscuraLua.Logger.LogLevel.Error;
local fc = 0;
for i, filename in ipairs(scandir(testdir)) do
	...
	print(ObscuraLua.colors("[CURRENT] ", "magenta") .. filename);
	for name, preset in pairs(presets) do
		...
		pipeline = ObscuraLua.Pipeline:fromConfig(preset);
		...
		if funcb == nil then
			print(ObscuraLua.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name .. ", Invalid Lua!");
			...
			if not validated then
				print(ObscuraLua.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name);
				print("[OUTA]    ",    outa);
				print("[OUTB]    ", outb);
				print("[SOURCE]", obfuscated);
				fc = fc + 1;
...
if fc < 1 then
	print(ObscuraLua.colors("[PASSED]  ", "green") .. "All tests passed!");
	return 0;
else
	print(ObscuraLua.colors("[FAILED]  ", "red") .. "Some tests failed!");
	if ciMode then
		error("Test Failed!")
	end
	return -1;
